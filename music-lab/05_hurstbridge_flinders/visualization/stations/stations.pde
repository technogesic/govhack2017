...
// image
PImage mapImage;
String mapImageFile = "img/train_lrgr@2x.png";
...
class Station
{
  ...
  void drawComponents() {    
    ...
    // draw station
    textAlign(CENTER);
    textFont(createFont("OpenSans-Semibold", 40, true), 40);
    fill(textColor);
    text(name, x-stationTextWidth/2, stationY, stationTextWidth, stationHeight);
    
    // draw borough 
    textFont(createFont("OpenSans-Semibold", 32, true), 32);
    fill(secondaryTextColor);
    ...
    if (!borough.equals(borough_next)) {
      ...
      // draw borough text
      textFont(createFont("OpenSans-Semibold", 32, true), 36);
      textAlign(RIGHT);
