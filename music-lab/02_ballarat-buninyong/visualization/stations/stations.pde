...
color secondaryTextColor = #2CABE8;
...
// text
float boroughY = 180;
float boroughTextWidth = 400;
float stationY = 450;
float stationWidth = 280;
float stationTextWidth = 400;
...
// image
PImage mapImage;
String mapImageFile = "img/bus_19-2x.png";
...
void setup() {  
  ...
  createFont("OpenSans-Semibold", 22, true);
  createFont("OpenSans-Semibold", 30, true);
  // noLoop();
}
...
class Station
{
  ...
  void drawComponents() {    
    ...
    image(mapImage, x-64, circle_y-64, 128, 128);
    ...
    // draw station
    textAlign(CENTER);
    textFont(createFont("OpenSans-Semibold", 16, true), 28);
    fill(textColor);
    text(name, x-stationTextWidth/2, stationY, stationTextWidth, stationHeight);
    
    // draw borough 
    textFont(createFont("OpenSans-Semibold", 60, true), 48);
    fill(secondaryTextColor);
    text(borough, x-boroughTextWidth/2, boroughY, boroughTextWidth, stationHeight);
    textAlign(CENTER);
    // draw label helper
    ...
    if (!borough.equals(borough_next)) {
      ...
      // draw borough text
      textFont(createFont("OpenSans-Semibold", 16, true), 48);
      textAlign(RIGHT);
