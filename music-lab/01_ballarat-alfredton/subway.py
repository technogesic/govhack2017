...
METERS_PER_BEAT = 50 # Higher numbers creates shorter songs
...
STATIONS_INPUT_FILE = 'data/stops.csv'
...
with open(STATIONS_INPUT_FILE, 'rb') as f:
	r = csv.reader(f, delimiter=',')
	next(r, None) # remove header
	for name,lat,lng,borough,income,playground,reserves,trees,bbqs in r:
		index = len(stations)
		stations.append({
			'index': index,
			'name': name,
	...
	for i in instruments_shelf:
		...
		# add to cart if in budget
			...
			if i['type'] != 'placeholder':
				...
				#print('Added Instrument: '+str(i['name']))
		# out of budget, finished
		else:
			break
	return instruments_cart

#Calculate Ambient Gains - Not very handsome indexing of correlation
def CalcAmbientGain(index):	
	gain_new = 0
	...
	if index <= 10:
		gain_new = 0.3
	elif index > 10 or index <= 100:
		gain_new = 0.1
	elif index > 100 or index <= 500:
		gain_new = 0.08
	elif index > 500 or index <= 750:
		gain_new = 0.05
	elif index > 750 or index <= 1000:
		gain_new = 0.025
	else:
		gain_new = 0
...
# Set Ambient Gains for instruments
def SetAmbientGains(station, instruments):
	playground = station['playground']
	reserves = station['reserves']
	trees = station['trees']
	bbqs = station['bbqs']

	for instrument in instruments:
		if instrument['type'] != 'ambient':
			continue
		else:
			if instrument['name'] == 'BBQ':
				#print('name: '+str(instrument['name']))
				#print('from_gain-intial: '+str(instrument['from_gain']))				
				instrument['from_gain'] = CalcAmbientGain(playground) + 0.2
				instrument['to_gain'] = instrument['from_gain'] - 0.2
				#print('from_gain-final: '+str(instrument['from_gain']))
				
			elif instrument['name'] == 'Children Playing':
				#print('name: '+str(instrument['name']))
				#print('from_gain-intial: '+str(instrument['from_gain']))
				instrument['from_gain'] = CalcAmbientGain(playground) + 0.2
				instrument['to_gain'] = instrument['from_gain'] - 0.2
				#print('from_gain-final: '+str(instrument['from_gain']))
			elif instrument['name'] == 'Forest Song':
				#print('name: '+str(instrument['name']))
				#print('from_gain-intial: '+str(instrument['from_gain']))
				instrument['from_gain'] = CalcAmbientGain(trees) - 0.1
				instrument['to_gain'] = instrument['from_gain'] - 0.2
				#print('from_gain-final: '+str(instrument['from_gain']))				
			else:
				#print('name: '+str(instrument['name']))
				#print('from_gain-intial: '+str(instrument['from_gain']))
				instrument['from_gain'] = CalcAmbientGain(reserves) + 0.1
				instrument['to_gain'] = instrument['from_gain'] - 0.1
				#print('from_gain-final: '+str(instrument['from_gain']))		
...
for index, station in enumerate(stations):
	...
	# set the ambient gains
	stations[index]['instruments'] = SetAmbientGains(stations[index], instruments)
	if index > 0:
